...
from compilation.enums import EstadoDAST, Region, VariableType, MethodType
...
from compilation.ast.specials import D_Assign, RiderNode, BikeNode
...
class Parser:
    def __init__(self):
        ...
        self.producciones = {"L": [["D", TokenType.T_SEMICOLON], ["@"], [TokenType.T_CLOSE_BRACE, "N"],
                                   ...
                                   [TokenType.T_ID, "H", TokenType.T_SEMICOLON]],
                             ...
                             "Z": [["e"], ["E", "U"]],
                             "I": [[TokenType.T_ASSIGN], [TokenType.T_ADD_AS], [TokenType.T_SUB_AS],
                                   [TokenType.T_DIV_AS],
                                   [TokenType.T_MUL_AS], [TokenType.T_MOD_AS], [TokenType.T_EXP_AS],
                                   [TokenType.T_AND_AS], [TokenType.T_OR_AS], [TokenType.T_XOR_AS]],
                             "Q": [[TokenType.T_S_VALUE], [TokenType.T_I_VALUE], [TokenType.T_D_VALUE],
                                   [TokenType.T_TRUE], [TokenType.T_FALSE], [TokenType.T_ID, "A"]],
                             ...
                             "Y": [["e"], [TokenType.T_MUL_OP, "M", "Y"], [TokenType.T_DIV_OP, "M", "Y"],
                                   [TokenType.T_MOD_OP, "M", "Y"], [TokenType.T_EXP_OP, "M", "Y"]],
                             "J": [["e"], ["O", "S"]],
        ...
        self.lista_producciones = [["L", "D", TokenType.T_SEMICOLON], ["L", "@"], ["L", TokenType.T_CLOSE_BRACE, "N"],
                                   ...
                                   ["L", TokenType.T_ID, "H", TokenType.T_SEMICOLON],
                                   ["D", TokenType.T_BOOL, TokenType.T_ID, TokenType.T_ASSIGN, "E"],
                                   ...
                                   ["Q", TokenType.T_S_VALUE], ["Q", TokenType.T_I_VALUE], ["Q", TokenType.T_D_VALUE],
                                   ["Q", TokenType.T_TRUE],
                                   ["Q", TokenType.T_FALSE], ["Q", TokenType.T_ID, "A"], ["C", TokenType.T_GREQ_REL],
                                   ["C", TokenType.T_GREAT_REL], ["C", TokenType.T_LESS_REL],
                                   ["C", TokenType.T_LEQ_REL], ["C", TokenType.T_EQ_REL], ["C", TokenType.T_NEQ_REL],
                                   ["M", "Q"], ["M", TokenType.T_OPEN_PAREN, "E", TokenType.T_CLOSE_PAREN],
                                   ["Y", "e"], ["Y", TokenType.T_MUL_OP, "M", "Y"], ["Y", TokenType.T_DIV_OP, "M", "Y"],
                                   ["Y", TokenType.T_MOD_OP, "M", "Y"], ["Y", TokenType.T_EXP_OP, "M", "Y"],
                                   ["J", "e"], ["J", "O", "S"], ["U", "e"],
                                   ["U", TokenType.T_COMMA, "E", "U"], ["X", TokenType.T_ADD_OP, "B", "X"],
                                   ["X", TokenType.T_SUB_OP, "B", "X"],
                                   ["X", "e"], ["E", "B", "X"], ["B", "M", "Y"], ["G", "E", "J"], ["S", "e"],
                                   ["S", TokenType.T_OR_OP, "G"], ["S", TokenType.T_AND_OP, "G"],
                                   ["S", TokenType.T_XOR_OP, "G"],
                                   ["^", "T", TokenType.T_ID, TokenType.T_ASSIGN,
                                    TokenType.T_OPEN_BRACKET, "E", "U", TokenType.T_CLOSE_BRACKET], ["H", "I", "E"],
                                   ["H", TokenType.T_OPEN_PAREN, "Z"
            ...
            if self.i < len(line):
                if self.i == 0:
                    if len(self.estados) != 0:
                        ...
                        if estado == Region.R_TYPE:
                            if line[self.i].token_type != TokenType.T_METHOD and line[
                                self.i].token_type != TokenType.T_CLOSE_BRACE and line[
                                self.i].token_type != TokenType.T_ID:
                            ...
                            if line[self.i].token_type == TokenType.T_METHOD or line[
                                self.i].token_type == TokenType.T_BIKE or line[self.i].token_type == TokenType.T_RIDER:
                                self.error = UnexpectedCharacterError("Wrong token in this scope", "",
                                                                      line[self.i].line, line[self.i].column)  #
                                return False
                            ...
                            if line[self.i].token_type == TokenType.T_METHOD or line[
                                ...
                                self.i].token_type == TokenType.T_BREAK:
                                self.error = UnexpectedCharacterError("Wrong token in this scope", "",
                                                                      line[self.i].line, line[self.i].column)  #
                                return False

                    else:
                        if line[self.i].token_type == TokenType.T_CONTINUE or line[
                            self.i].token_type == TokenType.T_BREAK or line[self.i].token_type == TokenType.T_RETURN:
                            self.error = UnexpectedCharacterError("Wrong token in this scope", "", line[self.i].line,
                                                                  line[self.i].column)  #
                            return False
                ...
                if termino == line[self.i].token_type:
                    if line[self.i].token_type == TokenType.T_RIDER or line[self.i].token_type == TokenType.T_BIKE:
                        ...
                        if self.estados[- 1] == Region.R_IF or self.estados[- 1] == Region.R_ELIF:
                            ...
                            self.error = UnexpectedCharacterError(
                                "The Elif region can only come after an IF region or an Elif region", "",
                                line[self.i].line, line[self.i].column)  #
                            return False
                        ...
                        if self.estados[- 1] == Region.R_IF or self.estados[- 1] == Region.R_ELIF:
                            ...
                            self.error = UnexpectedCharacterError(
                                "The Else region can only come after an IF region or an Elif region", "",
                                line[self.i].line, line[self.i].column)  #
                            return False
                    elif line[self.i].token_type == TokenType.T_CLOSE_BRACE:
                        if self.i + 1 == len(line) or (
                    ...
                    self.error = UnexpectedCharacterError("Wrong token in this string", "", line[self.i].line, line[
                        self.i].column)  # Aqui nunca va a entrar , solo si esta mal la matriz
                    return False
                ...
                if self.matriz[indice_nt][indice_t] is not None:
                    ...
                    if self.estadoDAST == EstadoDAST.EnExpresionAssign and termino != "A" and termino != "U" and termino != "Z" and termino != "H":
                        ...
                        if self.nodoactual.noderaiz.ast is not None:
                        ...
                        elif self.nodoactual.nododreconocimiento.padre is not None:
                            self.nodoactual.nododreconocimiento = self.nodoactual.nododreconocimiento.padre
                elif self.matriz[indice_nt][indice_t] != "e":
                    self.error = UnexpectedCharacterError("Wrong string in this language", "", line[self.i].line, line[
